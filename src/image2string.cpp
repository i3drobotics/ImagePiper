...
{
	unsigned int in_len = (unsigned int)encoded_string.size();
	int i = 0;
	int j = 0;
	int in_ = 0;
...
std::string Image2String::mat2str(const cv::Mat& m, int quality)
{
    uchar* result;
    std::vector<uchar> buf;
	bool floating_point = (m.type() == CV_32FC1 || m.type() == CV_32FC2 || m.type() == CV_32FC3);
    ...
    return base64_encode(result, (unsigned int)buf.size());
}
